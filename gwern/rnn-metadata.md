...
[Source](https://gwern.net/rnn-metadata)
...
RNN, NN sampling, poetry by AI, CLI, tutorial
...
As a baseline, a char-RNN with 2×2500 neurons, trained with 50% dropout, batch-size 55, and BPTT length 200, on the PG dataset without any author prefixes or suffixes, converges to a validation loss of ~1.08 after ~20 epochs.
...
## "The Complete Project Gutenberg Works of Jane Austen" https://www.gutenberg.org/ebooks/31100
...
## "Project Gutenberg Complete Works of Winston Churchill" https://www.gutenberg.org/ebooks/5400
...
## "The Entire Project Gutenberg Works of Mark Twain" https://www.gutenberg.org/ebooks/3200
...
Errored out of memory early the next day; the validation loss is still pretty meh, but at 1.1705321ya, can’t expect much, and indeed, the style is not impressive when I check several prefixes:
...
## delays to avoid anti-crawl defenses
wget 'https://www.gutenberg.org/ebooks/10031.txt.utf-8'  -O poe.txt && sleep 5s
...
for FILE in *.txt; do
  dos2unix $FILE;
  AUTHOR=$(echo $FILE | sed -e 's/\.txt//' | tr '[:lower:]' '[:upper:]')
  cat $FILE | tail -n +80 | head --lines=-362 | grep -i -v -e 'Gutenberg' -e 'http' -e 'file://' -e 'COPYRIGHT' -e 'ELECTRONIC VERSION' \
...
In a third try, I resumed from that checkpoint but increased the BPTT unrolling `seq_length` 50 → 210 to see if that would help it. It converged to 1.114 with suffixes still random. For a fourth try, I reduced dropout 0.5 → 0.1, which did not make a difference and converged to 1.117 after 8 epochs.
...
“Learning to Generate Reviews and Discovering Sentiment”, Radford et al 2017 (char-RNNs may discover untangled representations automatically)
- 

“Zero-Shot Style Transfer in Text Using Recurrent Neural Networks”, Carlson et al 2017
- 

“Hierarchical Neural Story Generation”, Fan et al 2018
- 

`textgenrnn` (a TensorFlow char-RNN implementation with models pretrained on large text corpuses, recent RNN architecture features like bidirectional RNNs & attention, optimizations, built-in context/metadata label support, and an interactive mode)
- 

“Better Language Models and Their Implications”, OA 2019 (GPT-2, a large-scale Transformer NN trained unsupervised to predict byte-pair by byte-pair akin to a char-RNN, can be used to solve many NLP tasks at SOTA or near-SOTA, by simply appending relevant tokens like “A:” to the input text and generating additional text)
- 
...
“RedCaps: web-curated image-text data created by the people, for the people”, Desai et al 2021 (subreddit-conditioned text caption style)
- 
...
wc -c data/geocities-corpus.txt
# 984248725
...
Still we see the same ‘bounce’ from better-than-`xz` predictive performance to 2–3 training loss. To check if it was size that was the problem, I went back to Aanand’s original 3×512 architecture:
...
After this data cleaning, I restarted training from the last checkpoint, same settings. 100,000 minibatches/4 epochs later, sampling still fails and validation loss is in the 100s! Restarting with higher dropout (0.8) didn’t help. Restarting with 0 dropout didn’t help either—after 50,000 minibatches, validation loss of 55.
...
Trying something intermediate between 512×3 and 3000×1, 200026ya×1, after 30k minibatches / 0.7 epochs, validation loss is ~0.98 and generated samples look good. So the larger flatter RNN is handling it better than the smaller deeper one.
...
Split out to separate article, “Finetuning the GPT-2-117M Transformer for English Poetry Generation”.
